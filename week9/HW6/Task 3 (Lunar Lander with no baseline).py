...
#     sample = random.random()
    ...
    return action, Categorical(action_dist).log_prob(action)  
...
def plot_durations():
    plt.figure(2)
    plt.clf()
    durations_t = torch.tensor(episode_points, dtype=torch.float)
    plt.title('Training...')
    plt.xlabel('Episode')
    plt.ylabel('Points')
    ...
    if len(durations_t) >= 100:
        ...
        global max_averaged_points
        max_averaged_points = max(means)
...
def optimize_model(state, reward, logprobs):
...
#         value_list.append(value)
    ...
    optimize_model(state_list, reward_list, logprobs_list)
            
#     if i_episode % TARGET_UPDATE == 0:
#         target_net.load_state_dict(policy_net.state_dict())

print("Max averaged reward: " + str(max_averaged_points))
