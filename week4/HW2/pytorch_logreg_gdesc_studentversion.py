...
class logreglayer(nn.Module):
  def __init__(self,dims):
    ...
    self.bias=torch.nn.Parameter(data=torch.zeros(1),requires_grad=True) #torch.nn.Parameter is a subclass of Tensor which is marked in a neural network module as parameter to be optimized, this is useful 1. for setting gradients to zero. 2. when using optimizer classes to tell them what parameters are optimized (next lecture)
...
def run():
  ...
  dtr= torch.utils.data.TensorDataset(torch.from_numpy(xtr).type(torch.FloatTensor), torch.from_numpy(ytr).type(torch.FloatTensor))
  # TensorDataset from tensors from xv, yv - our validation features and labels
  dv= torch.utils.data.TensorDataset(torch.from_numpy(xv).type(torch.FloatTensor), torch.from_numpy(yv).type(torch.FloatTensor))
  ...
  #TODO
  lossfunction=torch.nn.BCELoss() # which loss function suits here, given that our model produces 1-dimensional output  and we want to use it for classification?
  ...
  for epoch in np.arange(maxnumepochs):
    ...
    for ct,data in enumerate(loadertr):
      ...
      #TODO
      model.w.data-=learningrate*model.w.grad # learningrate/decay/momemntum is like alpha, learningrate defines the adjustment size
      model.bias.data-=learningrate*model.bias.grad
